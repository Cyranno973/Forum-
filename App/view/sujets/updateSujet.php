...
<div class="main-content">
	...
	<form action="index.php?action=goHandlingUpdateSujet&id=<?= $varRubricUser['id_sujet']; ?>" method="POST" >
	<div class="controls">
        <select multiple="" name="fname" id="select-rubric" class="form-control chosen-select" required>
		<option value="<?= $varRubricUser['id_rubric']; ?>"><?= $varRubricUser['title_rubric']; ?> : (rubrique actuelle)</option>
	...
	<label for="nameSujet" class="labelSujet">Titre du Sujet :</label>
		<input type="text" name="nameSujet" id="nameSujet" value="<?= $varCheckRubric['title_sujet']; ?>">
		<div id="tini">
			<textarea class="tinymce" name="sujetContent" id="sujetContent"><?= $varCheckRubric['content']; ?></textarea>
